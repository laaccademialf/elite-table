...
export const HomeView = () => {
  ...
  return (
    <main className="max-w-7xl mx-auto px-6 py-8 animate-in fade-in duration-700">
      {/* Hero Section */}
      <div className="mb-6 bg-gradient-to-br from-[#FFF7ED] to-[#F8E9D2] p-3 md:p-4 rounded-xl shadow border border-[#F3E5C8] transition-all duration-300">
        <div className="flex flex-col md:flex-row items-center md:items-center justify-between gap-4 min-h-[110px]">
          {/* Header and DatePicker */}
          <div className="flex-1 flex flex-col items-center md:items-start gap-1 md:gap-2 px-1 md:px-4">
            <h2 className="text-xl md:text-2xl font-extrabold text-slate-900 uppercase tracking-tight drop-shadow-sm">ВАША ОСОБЛИВА ПОДІЯ</h2>
          ...
          <div className="flex flex-col items-center md:items-end gap-2 md:pr-2">
            <div className="flex items-center gap-2 text-sm font-semibold text-slate-900">
              ...
              <a href="tel:+380443338948" className="hover:text-[#C5A059] transition">+38 (044) 333-8948</a>
            </div>
            <div className="flex items-center gap-3">
              <span className="px-2 py-1 text-[11px] md:text-xs font-semibold text-slate-800 bg-white/80 rounded-full border border-[#F3E5C8]">Чарівне у соцмережах</span>
      ...
      <div className="sticky top-16 z-40 bg-white/95 backdrop-blur py-6 -mx-6 px-6 mb-8 shadow-sm overflow-visible">
        <div className="flex justify-center gap-4 max-w-7xl mx-auto overflow-x-auto overflow-y-visible pb-2 md:flex-wrap md:pb-0 md:overflow-visible scroll-smooth" style={{scrollBehavior: 'smooth', scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
          ...
          `}</style>
          <button
            className={`group relative p-0 rounded-2xl w-24 h-24 md:w-28 md:h-28 flex-shrink-0 border-[3px] ${selectedCategory === null ? 'border-slate-900' : 'border-[#C5A059] hover:border-slate-900'} bg-white overflow-hidden transition`}
            onClick={() => setSelectedCategory(null)}
            aria-label="Всі категорії"
          >
            <div className="absolute inset-0 flex items-center justify-center text-2xl">📋</div>
            <div className="absolute inset-x-0 bottom-0 bg-white/80 text-slate-900 text-[9px] md:text-[11px] font-bold uppercase text-center px-1.5 py-1 leading-tight min-h-6">
              Всі
            </div>
          </button>
          {categories.length > 0 && categories.map((category) => (
            <button
              key={category.id}
              className={`group relative p-0 rounded-2xl w-24 h-24 md:w-28 md:h-28 flex-shrink-0 border-[3px] ${selectedCategory === category.name ? 'border-slate-900' : 'border-[#C5A059] hover:border-slate-900'} bg-white overflow-hidden transition`}
              onClick={() => {
